...
export const AccountPayableForm: React.FC<AccountPayableFormProps> = ({
  ...
  return <form onSubmit={handleSubmit} className="space-y-4">
      ...
      }))} placeholder="Ex: Energia Elétrica - Janeiro" required className="bg-slate-800" />
      </div>
      ...
      }))} placeholder="Ex: CEMIG" required className="bg-slate-800" />
      </div>
      ...
      }))} placeholder="0,00" required className="bg-slate-800" />
      </div>
      ...
      }))} required className="bg-gray-800" />
      </div>
      ...
      <div className="flex items-center space-x-2 bg-slate-700">
        <Checkbox id="has_boleto" checked={formData.has_boleto} onCheckedChange={checked => setFormData(prev => ({
        ...prev,
        has_boleto: !!checked
      }))} />
      ...
      }))} placeholder="Observações adicionais..." rows={3} className="bg-slate-800" />
      </div>

      <div className="flex gap-3 pt-4">
        <Button type="button" variant="outline" onClick={onCancel} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-50">
          Cancelar
        </Button>
